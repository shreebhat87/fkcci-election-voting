...
class MasterDataController extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('admin/master_data', [
            ...
            'excelHistory' => $batches->recent('members'),
    ...
    public function previewExcel()
    {
        ...
        [$rows, $errors, $columnsFound] = $this->parseExcel($tmpDir . $storedName);
    ...
    public function commitExcel()
    {
        ...
        [$rows, $errors] = $this->parseExcel($path);
        ...
        foreach ($rows as $row) {
            ...
            if ($existingByRfid && $existingByRfid['member_id'] !== $row['member_id']) {
                $commitErrors[] = "Row {$row['row_num']}: RFID {$row['rfid_tag']} already belongs to member {$existingByRfid['member_id']}.";
                continue;
            }
    ...
    /**
     * Parse and validate an Excel/CSV file. Returns [validRows, errorMessages, columnsFoundOrNull].
     * A "valid" row has all required fields and a RFID/Member ID that isn't
     * duplicated elsewhere *within this same file* (cross-file conflicts
     * against the DB are checked at commit time, not here).
     */
    private function parseExcel(string $path): array
    {
        ...
        foreach ($sheetRows as $i => $raw) {
            ...
            $missing = array_filter(self::REQUIRED_COLUMNS, static fn ($f) => $data[$f] === '');
            if ($missing) {
                $errors[] = "Row {$rowNum}: missing " . implode(', ', $missing) . '.';
                continue;
            }
            if (isset($seenRfid[$data['rfid_tag']])) {
                $errors[] = "Row {$rowNum}: duplicate RFID tag {$data['rfid_tag']} (also row {$seenRfid[$data['rfid_tag']]} in this file).";
                continue;
            }
            if (isset($seenMemberId[$data['member_id']])) {
                $errors[] = "Row {$rowNum}: duplicate Member ID {$data['member_id']} (also row {$seenMemberId[$data['member_id']]} in this file).";
                continue;
            }
            if ($data['email'] !== '' && ! filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
                $errors[] = "Row {$rowNum}: invalid email '{$data['email']}'.";
                continue;
            }

            $seenRfid[$data['rfid_tag']] = $rowNum;
            $seenMemberId[$data['member_id']] = $rowNum;
            $rows[] = $data;
        }

        return [$rows, $errors, $columnsFound];
    }
